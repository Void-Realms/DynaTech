...
public class LiquidTank extends SlimefunItem implements NotPlaceable {
    ...
    private ItemUseHandler onRightClick() {
        return e -> {
            ...
            Block b = e.getPlayer().getTargetBlock(null, 3);

            Optional<SlimefunItem> item = e.getSlimefunItem();
            ItemStack itemStack = e.getItem();

            if (item.isPresent()
                && item.get() instanceof LiquidTank
                && Slimefun.getProtectionManager().hasPermission(e.getPlayer(), b,
                Interaction.PLACE_BLOCK)
            ) {
                Block liquid = b.getRelative(e.getClickedFace());
                ...
                if (liquid.getType() == Material.AIR
                    && fluidName != null
                ...
                } else if ((liquid.getType() == Material.LAVA
                    || liquid.getType() == Material.WATER)
                    && fluidName != null
                    && fluidAmount <= liquidTank.getMaxLiquidAmount()
                    && liquid.isLiquid()
                ) {

                    Levelled liquidData = (Levelled) liquid.getBlockData();

                    if (liquidData.getLevel() == 0) {
    ...
    public void removeLiquid(ItemStack item, String fluidName, int fluidAmount) {
        ...
        int resultFluidAmount = itemFluidAmount - fluidAmount;
        if (itemFluidName != null
            && itemFluidName.equals(fluidName)
            && itemFluidAmount != 0
            && resultFluidAmount > 0
        ) {
            setLiquid(item, fluidName, resultFluidAmount);
